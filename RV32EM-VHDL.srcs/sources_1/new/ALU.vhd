...
-- Last update : Fri Jul  3 22:55:53 2020
-- Platform    : N/A
...
entity ALU is
	port (
		...
		-- control(4 downto 2) : funct3
		-- control(1) : Inst(30) ou funct7_5
...
begin
	...
	main_process : process(opA, opB_sig, funct3, funct7_5, opcode_5) begin
		if funct3 = "000" then -- add and sub
			if opcode_5 = '1' and funct7_5 = '1' then
				ALU_out <= std_logic_vector(signed(opA) - signed(opB_sig));
			else
				ALU_out <= std_logic_vector(signed(opA) + signed(opB_sig));
			end if;
		elsif funct3(1 downto 0) = "01" then -- shift
			if funct3(2) = '0' then
		...
		elsif funct3 = "010" then -- ser less then signed
			if signed(opA) < signed(opB_sig) then
				ALU_out <= x"00000001";
			else
				ALU_out <= x"00000000";
			end if;
		elsif funct3 = "011" then -- ser less then unsigned
